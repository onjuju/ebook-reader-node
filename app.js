...
/**
 * @param {Array} results 全部图书
 * @param {Number} key 数组下标
 * @description 通过key获得对应的图书, 并做一些处理
 */
...
app.get('/book/home', (req, res) => {
  ...
  conn.query('select * from book where cover != \'\'', (err, results) => {
    ...
    const banner = constant.resUrl + '/home_banner.jpg' // 封面图片
    const recommend = []
    const featured = [] // 精选
    const random = []
    // 随机推荐的分类 每个分类四本书
